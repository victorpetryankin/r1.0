		...
		Если ОшибокКомпонентов > 0 Тогда 
			...
			Если НЕ ЧастьОтчета.Криптокомпонент.Подключен Тогда 
				...
				ИначеЕсли ЧастьОтчета.Криптокомпонент.КодОшибки = 2 Тогда
					...
					ПредставленияКП.Вставить(Перечисление_ТипыКриптоПровайдеров.CryptoPro, "КриптоПро CSP");
					...
					Если КП_2 <> Неопределено Тогда 
						...
						Для Каждого Эл из РезультатыДиагностики.Отчет.Криптопровайдеры Цикл 
							Если Эл.Представление = ПараметрыКП.Имя Тогда 
								Продолжить;
							КонецЕсли;
							КП_1 = КонтекстМодуля.НайтиКриптопровайдераПоИмени(Эл.Представление);
						КонецЦикла;					
...
Функция ЗаполнитьОбщиеПараметры(ПараметрыСервера)
	...
	Если Не ПараметрыСервера.Организация.Свойство("НаименованиеСокращенное", НаименованиеСокращенное) Тогда 
		Если Не ПараметрыСервера.Организация.Свойство("КраткоеНаименование", НаименованиеСокращенное) Тогда 
			Если Не ПараметрыСервера.Организация.Свойство("Наименование", НаименованиеСокращенное) Тогда 
				НаименованиеСокращенное = "";
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	ПараметрыСервера.Организация.Вставить("НаименованиеСокращенное", НаименованиеСокращенное);	
...
Функция ПолучитьИнформациюФайлаАвтонастройки()
	...
	Криптопровайдеры = Новый Соответствие;
	Криптопровайдеры.Вставить(ПеречислениеТипыКриптоПровайдеров.VipNet, "Infotecs Cryptographic Service Provider");
	Криптопровайдеры.Вставить(ПеречислениеТипыКриптоПровайдеров.CryptoPro, "Crypto-Pro GOST R 34.10-2001 Cryptographic Service Provider");
	
	Криптопровайдеры2 = Новый Соответствие;
	Криптопровайдеры2.Вставить(ПеречислениеТипыКриптоПровайдеров.VipNet, "ViPNet CSP");
	Криптопровайдеры2.Вставить(ПеречислениеТипыКриптоПровайдеров.CryptoPro, "КриптоПро CSP");
		
	ДополнительныеРеквизитыУчетнойЗаписи = РезультатыДиагностики.Параметры.ДополнительныеРеквизитыУчетнойЗаписи;
	
	Если ДополнительныеРеквизитыУчетнойЗаписи <> Неопределено Тогда 
		ВладелецЭПИмяКриптопровайдера = Криптопровайдеры.Получить(ДополнительныеРеквизитыУчетнойЗаписи.ТипКриптопровайдера);
		ВладелецЭПИмяКриптопровайдера2 = Криптопровайдеры2.Получить(ДополнительныеРеквизитыУчетнойЗаписи.ТипКриптопровайдера);
...
Функция ПолучитьСписокПроблемСДоверенностями(ОрганизацияСсылка, ВладелецЭП, СведенияОРегистрации, СертификатВыданНаПредставителя)
	...
	Если Не ЗначениеЗаполнено(ВладелецЭП_ФЛ) И (ВладелецЭП_ФЛ = Неопределено) Тогда 
		Возврат Неопределено;
	КонецЕсли;
...
Функция ВыполнитьПроверкуПодключений() //2
	...
	Адрес = КонтекстЭДОКлиент.СерверПриемаОтчетностиФСС(ВернутьТипДокументовФСС(2));
	Порт = 443;
	Урл = "/WSInsurerCrypto/FileOperationsLnPort";
...
Процедура МенеджерКриптографииПолучитьКриптопровайдеровРезультат(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Выполнено Тогда 
		Поддерживаемые = КриптографияЭДКОКлиентСервер.ПоддерживаемыеКриптопровайдеры();
		...
		Для Каждого Криптопровайдер из Результат.Криптопровайдеры Цикл 
			...
			КонецЕсли;
			Найден = Ложь;
			Для Каждого Провайдер из Поддерживаемые Цикл 
				Если Криптопровайдер.Имя = Провайдер.Имя Тогда 
					Найден = Истина;
					Прервать;
				КонецЕсли;
			КонецЦикла;
			Если Найден Тогда 
				РезультатыПодключенияКК.Криптопровайдеры.Добавить(Криптопровайдер.Тип, Криптопровайдер.Имя);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция ОтправитьОтчетНаСервере(ОтчетДиагностики, ОписаниеОшибки = "", Знач ТолькоСформировать = Ложь)
	...
	Отчет_Абонент_УчетнаяЗапись.Криптопровайдер = ОтчетДиагностики.Абонент.УчетнаяЗапись.КриптопровайдерXML;
...
Функция ТипКлиента(ТипЧислом = -1) Экспорт
	...
	#ИначеЕсли МобильноеПриложениеКлиент Тогда 
		ТипЧислом = 4;
		Возврат "Мобильное приложение";
	#ИначеЕсли ВнешнееСоединение Тогда 
		ТипЧислом = 5;
		Возврат "Внешнее соединение";
	#КонецЕсли
	Возврат "Не удалось определить тип клиентского подключения";
...
Функция ПолучитьКриптопровайдераОтчета(Отчет, Автонастройка, ПараметрыКП = Неопределено)
	...
	Если Отчет.Проверки.ПроверкаУчетнойЗаписи.ИнформацияЛичногоСертификата.Криптопровайдер = Неопределено Тогда 
		Если Автонастройка.ВладелецЭП.КриптопровайдерENUM = ПредопределенноеЗначение("Перечисление.ТипыКриптоПровайдеров.CryptoPro") Тогда 
			ПараметрыКП = CryptoProCSP;
		ИначеЕсли Автонастройка.ВладелецЭП.КриптопровайдерENUM = ПредопределенноеЗначение("Перечисление.ТипыКриптоПровайдеров.VipNet") Тогда 
			ПараметрыКП = ViPNetCSP;
		КонецЕсли;
	...
	Если (CryptoProCSP.Имя = НеизвестныйКП.Имя И CryptoProCSP.Тип = НеизвестныйКП.Тип) Тогда 
		ПараметрыКП = CryptoProCSP;
		Возврат ПредопределенноеЗначение("Перечисление.ТипыКриптоПровайдеров.CryptoPro");
	КонецЕсли;
	Если (ViPNetCSP.Имя = НеизвестныйКП.Имя И ViPNetCSP.Тип = НеизвестныйКП.Тип) Тогда 
		ПараметрыКП = ViPNetCSP;
