...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли (ИмяСобытия = "Запись_ФизическиеЛица"
		...
		Если ЭтоРуководитель ИЛИ ЭтоБухгалтер ИЛИ ЭтоСотрудник Тогда
...
&НаКлиенте
Процедура СсылкаПаспортОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	ВыполнитьДействиеСФайлом(
		НавигационнаяСсылкаФорматированнойСтроки, 
		СтандартнаяОбработка, 
		"Паспорт", 
		Истина);
...
&НаКлиенте
Процедура ОчиститьСсылкаПаспортНажатие(Элемент)
...
&НаКлиенте
Процедура СсылкаСвидетельствоОПостановкеНаУчетОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
&НаКлиенте
Процедура СсылкаСНИЛСОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ОчиститьСНИЛСНажатие(Элемент)
	ОчиститьФайл(ЭтотОбъект, "СНИЛС");
КонецПроцедуры

&НаКлиенте
Процедура СсылкаПодтверждениеПолномочийВладельцаЭПОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ОчиститьСсылкаСвидетельствоОПостановкеНаУчетНажатие(Элемент)
	ОчиститьФайл(ЭтотОбъект, "СвидетельствоОПостановкеНаУчет")
КонецПроцедуры

&НаКлиенте
Процедура ОчиститьСсылкаПодтверждениеПолномочийВладельцаЭПНажатие(Элемент)
	ОчиститьФайл(ЭтотОбъект, "ПодтверждениеПолномочий");
КонецПроцедуры

&НаКлиенте
Процедура СсылкаСканаЗаявленияОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ОчиститьСсылкаСканаЗаявленияНажатие(Элемент)
...
&НаКлиенте
Процедура ОчиститьСсылкаДоверенностьНажатие(Элемент)
	
	ОчиститьФайл(ЭтотОбъект, "Доверенность");
	
КонецПроцедуры

&НаКлиенте
Процедура СсылкаДоверенностьОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
Процедура ЗаполнитьЗаявлениеДаннымиИз1СКонтрагент()
	...
	ЗаполнятьКраткоеНаименование 	= ЭтоЮридическоеЛицо И (НЕ ЗначениеЗаполнено(КраткоеНаименование) ИЛИ ЗаполнятьПринудительно);
...
&НаКлиенте
Процедура ВыполнитьДействиеСФайлом(
		НавигационнаяСсылкаФорматированнойСтроки, 
		СтандартнаяОбработка, 
		ВидДокумента, 
		ОдинФайл = Ложь)
		...
		Если ОписаниеФайла.Размер > МаксимальныйРазмерФайла Тогда
			ДлительнаяОтправкаКлиентСервер.ВывестиОшибку(
				СтрШаблон(НСтр("ru = 'Размер файла ""%1"" (%2) не соответствует требованиям: максимальный размер файла - %3.'"),
					...
					ОбщегоНазначенияЭДКОКлиентСервер.ТекстовоеПредставлениеРазмераФайла(МаксимальныйРазмерФайла)));
			Возврат;
		КонецЕсли;
	КонецЦикла;
	
	// Типы
	ТипыФайлов = СтрРазделить(ДопустимыеТипыФайлов, ";");
		...
		Если ТипыФайлов.Найти(ОписаниеФайла.Расширение) = Неопределено Тогда
			ДлительнаяОтправкаКлиентСервер.ВывестиОшибку(
				СтрШаблон(НСтр("ru = 'Тип файла ""%1"" не соответствует требованиям: допустимые типы файлов - ""%2"".'"),
					ОписаниеФайла.Имя, СтрСоединить(ТипыФайлов, ", ")));
			Возврат;
		КонецЕсли;
	КонецЦикла;
...
Процедура ИзменитьОформлениеВладельцаЭП()
	...
	// Видимость надписи
	ПоказатьНадпись = ФИОВладельцаИз1СКонтрагента И ВладелецЭЦПТип = ПредопределенноеЗначение("Перечисление.ТипыВладельцевЭЦП.Руководитель");
	Элементы.НадписьВладелецЭЦП.Видимость = ПоказатьНадпись;
	Если ПоказатьНадпись Тогда
		Элементы.НадписьВладелецЭЦП.Заголовок = ВладелецЭЦПФамилия + " " + ВладелецЭЦПИмя + " " + ВладелецЭЦПОтчество;
	КонецЕсли;
	
	// Видимость полей ввода ФИО
	ПоказатьПоляФИО = НЕ ПоказатьНадпись И НЕ ЗначениеЗаполнено(ВладелецЭЦП) 
		И НЕ ВладелецЭЦПТип = ПредопределенноеЗначение("Перечисление.ТипыВладельцевЭЦП.ДругойСотрудник");
	Элементы.ВладелецЭЦПФИО.Видимость     = ПоказатьПоляФИО;
	
	// Видимость гиперссылка
	ПоказатьГиперссылку = НЕ ПоказатьПоляФИО И НЕ ПоказатьНадпись;
	Элементы.ДекорацияВладелецЭЦП.Видимость = ПоказатьГиперссылку;
	Если ПоказатьГиперссылку Тогда
		Если ЗначениеЗаполнено(ВладелецЭЦП) Тогда
	...
	ПоказыватьТриТипаСотрудников = ЭтоЮридическоеЛицо;
...
Процедура ИзменитьОформление1СЭДО()
	...
	Элементы.Недоступно1СЭДО.Видимость 	= НЕ ОтображатьПодключениеЭДО;
	Элементы.ПодключитьЭДО.Видимость 	= ОтображатьПодключениеЭДО;
	Элементы.ОткрытьПараметрыПодключения.Видимость = ОтображатьПодключениеЭДО;
...
Процедура ИнициализироватьПараметрыЭДО()
	...
	Если ОбщегоНазначения.РазделениеВключено() ИЛИ РежимРаботыСКлючами = 1 И ДоступнаЭлектроннаяПодписьВМоделиСервиса Тогда 
		// Облачная ЭП
...
Процедура ПолучитьКриптопровайдераИзПараметров(Параметры)
	...
	Если Параметры.Свойство("CryptoProCSPУстановлен") 
		...
		И Параметры.Свойство("КомпонентаДляРаботыСКриптографиейПодключена")
		И Параметры.Свойство("ВсеСертификаты") Тогда
...
Процедура ЗаполнитьЗаявлениеДаннымиИзБазы(ДанныеОрганизацииИОтветственныхЛиц, ИННОрганизации = "")
	...
	Если ЭтоЮридическоеЛицо И ОрганизацияИмеетПустоеНаименование Тогда
		ПоказыватьКраткоеНаименование = Истина;
		КраткоеНаименование = "";
	Иначе
		ПоказыватьКраткоеНаименование = Ложь;
	КонецЕсли;
...
Процедура ВосстановитьНаправленияПоУмолчаниюФНС()
	...
	КонтекстЭДОСервер = ДокументооборотСКОВызовСервера.ПолучитьОбработкуЭДО();
	КонтекстЭДОСервер.НовоеНаправление(ПолучателиФНС, ФНС, ДанныеОрганизации.КодНО, ДанныеОрганизации.КППЮЛ);
...
Процедура ИзменитьОформлениеИНН(ЭтоИзменениеИНН = Ложь)
	...
	Если ИспользуетсяОднаОрганизация И НЕ ЗначениеЗаполнено(ИНН) Тогда
		Элементы.ЗаголовокИНН.Заголовок = НСтр("ru = 'Заполните ИНН:'");
	КонецЕсли;
...
Процедура ЗаполнитьЗаявлениеДаннымиИзСкопированногоЗаявления()
	...
	ПоказыватьИНН = НЕ ЗначениеЗаполнено(СокрЛП(ИНН));
	Если ЭтоЮридическоеЛицо И КраткоеНаименование = "" Тогда
		ПоказыватьКраткоеНаименование = Истина;
	Иначе
		ПоказыватьКраткоеНаименование = Ложь;
	КонецЕсли;
...
Процедура УстановитьНастройкиРежимаОграниченнойФункциональности()
	...
	// ЭтоРежимБесплатнойНулевойОтчетности = Ложь;
...
Процедура ИзменитьОформлениеТелефонаИПочтыДляПаролей(Форма)
	...
	Элементы.ПодсказкаДляТелефонаДляПаролей.Видимость = Форма.РежимРаботыСКлючами = 1;
	Элементы.ПодсказкаЭлектроннаяПочтаДляПаролей.Видимость = Форма.РежимРаботыСКлючами = 1;
...
Процедура ИзменитьОформлениеСпособаПодписания(Форма)
	...
	Иначе
		...
		Если Форма.ЗапретитьИзменение Тогда
			...
			Элементы.ПричинаНевозможностиПодписания.Видимость = Ложь;
		
		ИначеЕсли ОтсуствуетКриптопровайдер Тогда
			
			Элементы.ПояснениеОНевозможностиЭлектронногоПодписания.Заголовок = Новый ФорматированнаяСтрока(
				НСтр("ru = ' Необходимо установить криптопровайдер. '"),
				Новый ФорматированнаяСтрока(НСтр("ru = 'Установить'"),,,,"Установить криптопровайдер"));
			
			Подсказка = Новый ФорматированнаяСтрока(НСтр("ru = 'Подписание электронной подписью доступно только при наличии программы защиты информации'"));
			ИзменитьОформлениеВариантовПодписания_ВывестиПодсказку(Подсказка, Элементы, Ложь);
			
		ИначеЕсли ЭтоКонфликтКриптопровайдеров Тогда
			
			Элементы.ПояснениеОНевозможностиЭлектронногоПодписания.Заголовок = Новый ФорматированнаяСтрока(
				НСтр("ru = ' Недоступно при конфликте криптопровайдеров. '"),
				Новый ФорматированнаяСтрока(НСтр("ru = 'Исправить'"),,,,"Исправить"));
			
			Подсказка = Новый ФорматированнаяСтрока(НСтр("ru = 'Подписание электронной подписью недоступно при конфликте криптопровайдеров.'"));
			ИзменитьОформлениеВариантовПодписания_ВывестиПодсказку(Подсказка, Элементы, Ложь);
		...
		ИначеЕсли НЕ Форма.CryptoProCSPУстановлен И НЕ Форма.ViPNetCSPУстановлен И НЕ Форма.КомпонентаДляРаботыСКриптографиейПодключена Тогда
			...
			Подсказка = Новый ФорматированнаяСтрока(Форма.ТекстОшибкиПодключенияКомпоненты);
				
			ИзменитьОформлениеВариантовПодписания_ВывестиПодсказку(Подсказка, Элементы, Ложь);
			
		ИначеЕсли ЗначениеЗаполнено(Форма.Организация) И НЕ РезультатПроверкиВладелецЭЦП.ЕстьОшибка Тогда
			...
			Элементы.ПояснениеОНевозможностиЭлектронногоПодписания.Заголовок = НСтр("ru = ' Не найдены действующие сертификаты'");
			
			ИзменитьОформлениеВариантовПодписания_ВывестиПодсказку(Подсказка, Элементы);
			
		ИначеЕсли ЗначениеЗаполнено(Форма.Организация) И РезультатПроверкиВладелецЭЦП.ЕстьОшибка Тогда
			
			Элементы.ПояснениеОНевозможностиЭлектронногоПодписания.Заголовок = НСтр("ru = ' Не указан владелец эл. подписи или его ФИО'");
			
			ПодстрокаНачало 	 = НСтр("ru = 'Не удалось найти действующий сертификат по организации '");
			ПодстрокаОрганизация = Новый ФорматированнаяСтрока(Строка(Форма.Организация),,,,ПолучитьНавигационнуюСсылку(Форма.Организация));
			ПодстрокаКонец	 	 = НСтр("ru = ' поскольку не указан владелец эл. подписи или его ФИО'");
...
&НаКлиентеНаСервереБезКонтекста
Процедура ИзменитьОформлениеВариантовПодписания_ВывестиПодсказку(Подсказка, Элементы, ВыводитьСсылкуНавсеСертификаты = Истина)
	
	Если ВыводитьСсылкуНавсеСертификаты Тогда
		
		ПодстрокаКоманда = Новый ФорматированнаяСтрока(НСтр("ru = 'Посмотреть все сертификаты'"),,,,"e1cib/command/ОбщаяКоманда.ПоказатьВсеЛичныеСертификаты");
	...
	Элементы.ПодсказкаОНевозможностиЭлектронногоПодписания.РасширеннаяПодсказка.Заголовок = Подсказка;
