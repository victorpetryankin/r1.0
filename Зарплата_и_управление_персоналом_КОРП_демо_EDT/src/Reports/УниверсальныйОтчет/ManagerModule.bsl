...
Функция ТекстЗапросаПоМетаданным(ПараметрыОтчета)
	...
	ТекстЗапроса = ТекстЗапроса
		+ " * ИЗ " + ИмяТипаДанных + "." + ПараметрыОтчета.ИмяОбъектаМетаданных;
	...
	Если НЕ ПустаяСтрока(ПараметрыОтчета.ИмяТаблицы) Тогда
		...
		Если ИмяТипаДанных = "Справочник" Или ИмяТипаДанных = "Документ" 
			...
			МетаданныеОбъекта = Метаданные[ПараметрыОтчета.ТипОбъектаМетаданных][ПараметрыОтчета.ИмяОбъектаМетаданных];
			Если МетаданныеОбъекта.ТабличныеЧасти.Найти(ПараметрыОтчета.ИмяТаблицы) <> Неопределено Тогда
...
Функция ПараметрыОтчета(Настройки, ПользовательскиеНастройки) Экспорт
	...
	Если Настройки.ДополнительныеСвойства.Свойство("ОтчетИнициализирован")
		...
		Для Каждого Элемент Из ПользовательскиеНастройки.Элементы Цикл
			...
			Если ПараметрыОтчета.Свойство(ИмяПараметра, ЗначениеПараметра)
				И ЗначениеПараметра <> Неопределено Тогда 
				ПараметрыОтчета[ИмяПараметра] = ЗначениеПараметра;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
