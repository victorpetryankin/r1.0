...
Функция Сохранить(Автосохранение = Ложь, КодИФНС, КПП, Вариант, ВидДокументаНомерКорректировкиИзменен = Неопределено, НаимТекРаздела) Экспорт
	...
	Если ОбъектОтчета.ЗаписатьДокумент(СтруктураРеквизитовФормы.мВыбраннаяФорма, ПараметрыЗаписи) = Ложь Тогда
		...
		Если НЕ Автосохранение Тогда
			...
			Сообщение.Текст = ПараметрыЗаписи.ОшибкиВыгрузки[0];
			Сообщение.Сообщить();
		КонецЕсли;
...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	ИначеЕсли СтрНачинаетсяС(Область.Имя, "П00019М108005") Тогда
		ИмяОбл = "П00019М102001" + РегламентированнаяОтчетностьКлиентСервер.СтрокаСИндексомПоказателя(
		 ИндексПоказателя(Область.Имя) + 1);
...
Процедура ВставитьДополнительнуюСтроку()
	...
	Если СтруктураРеквизитовФормы.СтруктураМногострочныхЧастей.Свойство(ИмяГруппы) Тогда
		...
		ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы110М2 Тогда
			...
			+ РегламентированнаяОтчетностьКлиентСервер.СтрокаСИндексомПоказателя(
			 ИндексПоказателя(ИмяОбластиПреемника) + 1);
			Обл110 = ТабличныйДокумент.Области.Найти(ИмяПокСтр110);
			Обл110.Значение = Обл020.Значение;
...
Процедура ВывестиНаПечатьДанныеМногоуровневогоРаздела(СтруктураПараметров, ОбщиеРеквизитыРазделов, СтрПоля)
	...
	Если ИмяТекРаздела = "Раздел1" Тогда
		...
		МакетРаздела = ОбъектОтчета.ПолучитьМакет(Сред(ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + "Раздел1");
		...
		Для НомЛиста = 1 По КолЛистовРаздела1 Цикл
			...
			Для Ном = 1 По СтрокМнЧ1НаЛисте - 1 Цикл
				ИмяОбластиПреемника = "ДобавлениеСтроки"
				+ СтруктураРеквизитовФормы.мИдГруппы1М1 + РегламентированнаяОтчетностьКлиентСервер.СтрокаСИндексомПоказателя(Ном);
				ВставитьДопСтроку(СтруктураРеквизитовФормы.мИдГруппы1М1, ИмяОбластиПреемника);
			КонецЦикла;
			
			Для Ном = 1 По СтрокМнЧ2НаЛисте - 1 Цикл
				ИмяОбластиПреемника = "ДобавлениеСтроки"
				+ СтруктураРеквизитовФормы.мИдГруппы1М2 + РегламентированнаяОтчетностьКлиентСервер.СтрокаСИндексомПоказателя(Ном);
	...
	ИначеЕсли ИмяТекРаздела = "Раздел1Прил1" Тогда
		...
		МакетРаздел1Прил1 = ОбъектОтчета.ПолучитьМакет(Сред(ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + "Раздел1Прил1");
		
		МакетРаздел1Прил1Подр1_1и1_2 = ОбъектОтчета.ПолучитьМакет(Сред(ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + "Раздел1Прил1Подр1_1и1_2");
		
		МакетРаздел1Прил1Подр1_3_1 = ОбъектОтчета.ПолучитьМакет(Сред(ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + "Раздел1Прил1Подр1_3_1");
		
		МакетРаздел1Прил1Подр1_3_2 = ОбъектОтчета.ПолучитьМакет(Сред(ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + "Раздел1Прил1Подр1_3_2");
		
		МакетРаздел1Прил1Подр1_4 = ОбъектОтчета.ПолучитьМакет(Сред(ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + "Раздел1Прил1Подр1_4");
	...
	ИначеЕсли ИмяТекРаздела = "Раздел1Прил2" Тогда
		...
		МакетРаздела = ОбъектОтчета.ПолучитьМакет(Сред(ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + "Раздел1Прил2");
	...
	ИначеЕсли ИмяТекРаздела = "Раздел1Прил3"
		...
		МакетРаздела = ОбъектОтчета.ПолучитьМакет(Сред(ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + ИмяТекРаздела);
