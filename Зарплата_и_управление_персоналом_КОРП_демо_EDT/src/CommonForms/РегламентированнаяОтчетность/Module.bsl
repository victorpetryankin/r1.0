...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Получение контекста ЭДО" Тогда
		Возврат;
	КонецЕсли;
		...
		Если Поле.Имя = "ОтчетыСтатус" Тогда
			...
			Если РегламентированнаяОтчетностьКлиентСервер.ОбъектНеОтправлялсяЧерез1СОтчетность(
				...
				Если ТипЗнч(Ссылка)
					...
					"ru='Изменение вручную статуса электронных представлений регламентированных отчетов не предусмотрено.'");
					ПоказатьПредупреждение(, ТекстПредупреждения);
					
					Возврат;
					
				ИначеЕсли ТипЗнч(Ссылка)
					= Тип("СправочникСсылка.МакетыПенсионныхДел") Тогда
					
					ТекстПредупреждения = НСтр(
					"ru='Изменение вручную статуса макетов пенсионных дел не предусмотрено'");
					ПоказатьПредупреждение(, ТекстПредупреждения);
			...
			Для Каждого ЭлементОтбора Из ЭлементПользовательскойНастройки.Элементы Цикл
				Если ТипЗнч(ЭлементОтбора) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
					Если ЭлементОтбора.ЛевоеЗначение = ПолеКомпоновкиНаименованиеОтчета Тогда
						...
						ИначеЕсли СокрЛП(ЭлементОтбора.ПравоеЗначение) = "Макет пенсионного дела" Тогда
							ЗначениеВидОтчетаСтр = ЭлементОтбора.ПравоеЗначение;
						КонецЕсли;
...
Процедура УстановитьОтборы(Форма)
	...
	Если ТипЗнч(Форма.ВидОтчета) = Тип("СписокЗначений") Тогда
		Для Каждого ЭлементВидОтчета Из Форма.ВидОтчета Цикл
			Если СокрЛП(ЭлементВидОтчета.Значение) = СтрМакетПенсионногоДела Тогда
		...
		КонецЦикла;
	Иначе
		Если СокрЛП(Форма.ВидОтчета) = СтрМакетПенсионногоДела Тогда
