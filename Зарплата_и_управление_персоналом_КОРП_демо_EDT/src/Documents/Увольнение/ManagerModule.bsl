...
Функция ДанныеДляПроведения(РеквизитыДляПроведения, МенеджерВременныхТаблиц, ПериодРегистрации, Организация, ПеремещаемыеСовместители = Неопределено, СтруктураВидовУчета = Неопределено) Экспорт
	...
	Если СтруктураВидовУчета.ОстальныеВидыУчета Тогда
		...
		РасчетЗарплатыРасширенный.ЗаполнитьНачисления(ДанныеДляПроведения, РеквизитыДляПроведения.Ссылка, "Начисления,НачисленияПерерасчет,Пособия,ПособияПерерасчет,Льготы", "Ссылка.ПериодРегистрации", , , "ПериодРегистрации");
...
Функция РеквизитыДляПроведения(ДокументСсылка)
	...
	|			ТОГДА НАЧАЛОПЕРИОДА(Увольнение.ДатаУвольнения, МЕСЯЦ)
	|		КОГДА Увольнение.МесяцНачалаРасчетов = ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА ВЫБОР
	...
	|			ТОГДА НАЧАЛОПЕРИОДА(УвольнениеСпискомСотрудники.ДатаУвольнения, МЕСЯЦ)
	|		КОГДА УвольнениеСпискомСотрудники.МесяцНачалаРасчетов = ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА ВЫБОР
