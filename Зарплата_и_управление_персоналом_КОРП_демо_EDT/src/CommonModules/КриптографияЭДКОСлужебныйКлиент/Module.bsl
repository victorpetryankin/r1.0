...
Процедура ПолучитьСертификатыПослеСозданияМенеджераКриптографии(Результат, ВходящийКонтекст) Экспорт
	...
	Иначе
		...
		Иначе
			ЗавершитьСОтрицательнымРезультатом(ПолучитьОписаниеОшибки(Результат), ВходящийКонтекст);		
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗавершитьСОтрицательнымРезультатом(Ошибка, ВходящийКонтекст)
	
	ОбщегоНазначенияЭДКОКлиент.ЗавершитьСОтрицательнымРезультатом(Ошибка, ВходящийКонтекст, ИмяСобытия());
...
Процедура ПодписатьPKCS7(ОповещениеОЗавершении, Сертификат, ИмяФайлаДанных, ВключатьИсходныеДанные, ИмяФайлаПодписи, ВыводитьСоообщения, МенеджерКриптографии, ЭтоПодписаниеЗаявления) Экспорт
	...
	ВходящиеПараметры = ПараметрыВМассив(5, Сертификат, ИмяФайлаДанных, ВключатьИсходныеДанные, ИмяФайлаПодписи, ЭтоПодписаниеЗаявления);
...
Процедура Зашифровать(ОповещениеОЗавершении, СертификатОтправителя, ИмяФайлаДанных, СертификатПолучателя, ПараметрыКриптографии,
						КаталогРезультатаИлиАдреса, ВыводитьСоообщения, МенеджерКриптографии) Экспорт
...
Процедура ХешироватьДанныеПослеВыполнения(Результат, Параметры, ВходящийКонтекст) Экспорт
	...
	ВыполнитьОбработкуОповещения(
		ВходящийКонтекст.ОповещениеОЗавершении,
		КриптографияЭДКОКлиентСервер.ПодготовитьРезультат(Истина, "Хеш", Результат, ВходящийКонтекст));
...
Процедура ПредварительноСоздатьМенеджерКриптографии(ОповещениеОЗавершении, ИмяМетода, ВходящиеПараметры = Неопределено, ВыводитьСообщения = Истина, МенеджерКриптографии = Неопределено)
	...
	Если МенеджерКриптографии <> Неопределено Тогда
		Результат = КриптографияЭДКОКлиентСервер.ПодготовитьРезультат(Истина, "МенеджерКриптографии", МенеджерКриптографии);
...
Процедура СоздатьМенеджерКриптографии2(ОповещениеОЗавершении, Криптопровайдер = Неопределено, ДополнительныеПараметры = Неопределено, 
	ПослеУстановкиВнешнейКомпоненты = Ложь, ВыполнятьПроверкуКриптопровайдера = Истина) Экспорт
	...
	Иначе
		ЗавершитьСОтрицательнымРезультатом(
			КриптографияЭДКОСлужебныйКлиентСервер.ОшибкаОтсутствуетКомпонентаДляИспользуемогоКлиентскогоПриложения(), Контекст);
		Возврат;
	КонецЕсли;
...
Процедура СоздатьМенеджерКриптографии2ПослеПодключенияВнешнейКомпоненты(Подключена, ВходящийКонтекст) Экспорт
	...
	Если Подключена Тогда
		...
		Исключение
			ЗавершитьСОтрицательнымРезультатом(
				КриптографияЭДКОСлужебныйКлиентСервер.ОшибкаВнешняяКомпонентаПодключенаНоНеУдалосьСоздатьAddIn(), ВходящийКонтекст);
			Возврат;
		КонецПопытки;
	...
	Иначе
		Если ВходящийКонтекст.ПослеУстановкиВнешнейКомпоненты Тогда
			...
			Возврат;
		Иначе
			Если ВходящийКонтекст.ДополнительныеПараметры.ПредлагатьУстановкуВнешнейКомпоненты Тогда
...
Процедура СоздатьМенеджерКриптографии2ПослеУстановкиСвойстваНеВыводитьСообщенияОбОшибках(ВходящийКонтекст) Экспорт
	...
	Если ЗначениеЗаполнено(ВходящийКонтекст.Криптопровайдер) Тогда
		Если КриптографияЭДКОКлиентСервер.ЭтоПоддерживаемыйКриптопровайдер(ВходящийКонтекст.Криптопровайдер)
			И ВходящийКонтекст.ВыполнятьПроверкуКриптопровайдера Тогда
			Оповещение = Новый ОписаниеОповещения(
