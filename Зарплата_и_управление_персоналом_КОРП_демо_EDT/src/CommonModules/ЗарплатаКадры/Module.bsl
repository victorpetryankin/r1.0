...
Процедура УправлениеДоступомЗаполнитьСвойстваВидаДоступа(ВидыДоступа) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы") Тогда
		...
		Модуль.УправлениеДоступомЗаполнитьСвойстваВидаДоступа(ВидыДоступа);
	КонецЕсли;
...
Процедура ПриДобавленииКлассификаторов(Классификаторы) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ТаблицаКлассификаторов Цикл
		...
		Описатель.Идентификатор = СтрокаТаблицы.Идентификатор;
...
//
// Параметры:
//		ПолноеИмяОбъектаМетаданных             - Строка - полное имя объекта метаданных (см. НайтиПоПолномуИмени).
...
Функция ЭтоОбъектЗарплатноКадровойБиблиотекиПолной(ПолноеИмяОбъектаМетаданных, 
	...
	ИменаПодсистемБЗК = ИменаПодсистемБЗКВерхнегоУровня(
	...
	Если Метаданные.Подсистемы.Содержит(ОбъектМетаданных) 
		И ИменаПодсистемБЗК.Найти(ОбъектМетаданных.Имя) <> Неопределено Тогда
			Возврат Истина;	
	КонецЕсли;
	
	ЭтоОбъектБиблиотеки = ЗарплатаКадрыБазовый.ЭтоОбъектЗарплатноКадровойБиблиотеки(ПолноеИмяОбъектаМетаданных)
		ИЛИ ЗарплатаКадры.ОбъектМетаданныхВключенВПодсистемы(ПолноеИмяОбъектаМетаданных, СтрСоединить(ИменаПодсистемБЗК, ","));
...
Функция СтруктураАдресаИзXML(Адрес, ВидАдреса = Неопределено) Экспорт
	...
	СтруктураАдреса = РаботаСАдресами.ПредыдущаяСтруктураКонтактнойИнформацииXML(Адрес, ВидАдреса);
...
Функция ОбновитьКлассификатор(ТекстXML) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.РаботаСКлассификаторами") Тогда
		МодульРаботаСКлассификаторами = ОбщегоНазначения.ОбщийМодуль("РаботаСКлассификаторами");
		ЗагруженнаяВерсия = МодульРаботаСКлассификаторами.ВерсияКлассификатора(ЗагружаемыеСведения.Идентификатор, Истина);
		Если ЗагруженнаяВерсия >= ЗагружаемыеСведения.Версия Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	ТаблицаЗначений = ПолныеИменаВСсылки(ЗагружаемыеСведения.Данные, НаборЗаписей.ВыгрузитьКолонки());
...
//
Функция ПолныеИменаВСсылки(ТаблицаБезСсылок, ТаблицаСоСсылками) Экспорт
	...
	Для Каждого СтрокаТаблицыБезСсылок Из ТаблицаБезСсылок Цикл
		СтрокаТаблицыСоСсылками = ТаблицаСоСсылками.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаТаблицыСоСсылками, СтрокаТаблицыБезСсылок);
		...
		Для Каждого ИмяКолонки Из ИменаКолонокДляПреобразованияСсылок Цикл
			...
			Если СтрНачинаетсяС(ЗначениеБезСсылок, "ПредопределенноеЗначение:") Тогда
				...
				СтрокаТаблицыСоСсылками[ИмяКолонки] = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент(ПолноеИмяПредопределенного);
			ИначеЕсли СтрНачинаетсяС(ЗначениеБезСсылок, "НайтиПоКоду:") Тогда
				...
				Если Позиция > 0 Тогда
					...
					СтрокаТаблицыСоСсылками[ИмяКолонки] = Менеджер.НайтиПоКоду(Код);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
Функция СсылкиВПолныеИмена(ТаблицаСоСсылками) Экспорт
	...
	Для Каждого СтрокаТаблицыСоСсылками Из ТаблицаСоСсылками Цикл
		СтрокаТаблицыБезСсылок = ТаблицаБезСсылок.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаТаблицыБезСсылок, СтрокаТаблицыСоСсылками);
		
		Для Каждого ИмяКолонки Из ИменаКолонокДляПреобразованияСсылок Цикл
			Ссылка = СтрокаТаблицыСоСсылками[ИмяКолонки];
