...
Процедура ЗаполнитьОбщиеИсточникиДанныхОтчета(Объект, ДополнительныеПоляПредставлений = Неопределено, ТолькоРазрешенные = Истина) Экспорт
	...
	Если ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		Если ДополнительныеПоляПредставлений <> Неопределено
			...
			Для Каждого ОписаниеПредставления Из ДополнительныеПоляПредставлений Цикл
				
				Если ОписаниеПредставления.Значение.Количество() > 0 Тогда
					
					ВывестиДополнительныеПоляПредставленийВСКД(
						Объект.СхемаКомпоновкиДанных, ОписаниеПредставления.Значение, ОписаниеПредставления.Ключ);
...
Процедура ВывестиДополнительныеПоляПредставленийВСКД(СКД, ДополнительныеПоляПредставлений, ИмяТаблицыПредставления) Экспорт
	
	ВывестиДополнительныеПоляПредставленийВНаборыДанныхСКД(СКД, СКД.НаборыДанных, ДополнительныеПоляПредставлений, ИмяТаблицыПредставления);
...
Процедура ВывестиДополнительныеПоляПредставленийВЗапрос(Запрос, ДополнительныеПоляПредставлений, Знач ИмяТаблицыПредставления) Экспорт
	...
	ВывестиДополнительныеПоляПредставленийВТекстЗапроса(Запрос.Текст, ДополнительныеПоляПредставлений, ИмяТаблицыПредставления);
	
КонецПроцедуры

Процедура ВывестиДополнительныеПоляПредставленийВТекстЗапроса(ТекстЗапроса, ДополнительныеПоляПредставлений, Знач ИмяТаблицыПредставления) Экспорт
		...
		Если ТипЗнч(ЗапросПакета) = Тип("ЗапросВыбораСхемыЗапроса")
			...
			Для Каждого Оператор Из ЗапросПакета.Операторы Цикл
				...
				Для Каждого СтрокаОписанияПоля Из ДополнительныеПоляПредставлений Цикл
					...
					Если ЗапросПакета.Колонки.Найти(СтрокаОписанияПоля.ИмяПоля) = Неопределено Тогда
						
						Оператор.ВыбираемыеПоля.Добавить(СтрокаОписанияПоля.ПустоеЗначениеНаЯзыкеЗапросов);
						ЗапросПакета.Колонки[ЗапросПакета.Колонки.Количество() - 1].Псевдоним = СтрокаОписанияПоля.ИмяПоля;
							...
							Если ТипЗнч(Источник.Источник) = Тип("ТаблицаСхемыЗапроса") И Источник.Источник.ИмяТаблицы = ТаблицаИсточник.Ключ Тогда
								...
								Для Каждого СтрокаОписанияПоля Из ТаблицаИсточник.Значение Цикл
									...
									Если Не ПустаяСтрока(Псевдоним) Тогда
										...
										Если ЗапросПакета.Колонки.Найти(Новый ВыражениеСхемыЗапроса(ВыражениеПоля)) = Неопределено Тогда
											
											Оператор.ВыбираемыеПоля.Добавить(ВыражениеПоля);
											Если ЗапросПакета.Колонки.Найти(Псевдоним) = Неопределено Тогда
...
Процедура ВывестиДополнительныеПоляПредставленийВНаборыДанныхСКД(СКД, НаборыДанных, ДополнительныеПоляПредставлений, ИмяТаблицыПредставления, УстанавливатьПутиСКД = Истина)
	
	Для Каждого НаборДанных Из НаборыДанных Цикл
		
		Если ТипЗнч(НаборДанных) = Тип("НаборДанныхОбъединениеСхемыКомпоновкиДанных") Тогда
			ВывестиДополнительныеПоляПредставленийВНаборыДанныхСКД(СКД, НаборДанных.Элементы, ДополнительныеПоляПредставлений, ИмяТаблицыПредставления, Ложь);
		ИначеЕсли ТипЗнч(НаборДанных) = Тип("НаборДанныхЗапросСхемыКомпоновкиДанных") Тогда
			ВывестиДополнительныеПоляПредставленийВТекстЗапроса(НаборДанных.Запрос, ДополнительныеПоляПредставлений, ИмяТаблицыПредставления);
		КонецЕсли;
		
		ДобавитьПоляНаборуДанныхСКД(НаборДанных, ДополнительныеПоляПредставлений, УстанавливатьПутиСКД);
		
	КонецЦикла;
	
КонецПроцедуры

Процедура ДобавитьПоляНаборуДанныхСКД(НаборДанных, ДополнительныеПоляПредставлений, УстанавливатьПутиСКД)
	...
	Для Каждого СтрокаОписанияПоля Из ДополнительныеПоляПредставлений Цикл
		...
		Если СтрокаОписанияПоля.ПсевдонимыПолей.Количество() > 0 Тогда
			...
			Для Каждого ОписаниеПсевдонимаПоля Из СтрокаОписанияПоля.ПсевдонимыПолей Цикл
				...
				ДобавитьПолеВНаборДанныхСКД(НаборДанных, ОписаниеПсевдонима.ПсевдонимПоля, ОписаниеПсевдонима, УстанавливатьПутиСКД);
				
			КонецЦикла;
			
		Иначе
			ДобавитьПолеВНаборДанныхСКД(НаборДанных, СтрокаОписанияПоля.ИмяПоля, СтрокаОписанияПоля, УстанавливатьПутиСКД);
		КонецЕсли;
...
Процедура ДобавитьПолеВНаборДанныхСКД(НаборДанных, ИмяПоля, ОписаниеПоля, УстанавливатьПутиСКД)
