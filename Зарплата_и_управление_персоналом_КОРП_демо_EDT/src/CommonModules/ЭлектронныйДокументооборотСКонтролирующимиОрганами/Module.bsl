...
Процедура НачатьОпределениеТекстаЗаявления(АдресХранилища, Заявление) Экспорт
	
	КонтекстЭДОСервер = ДокументооборотСКОВызовСервера.ПолучитьОбработкуЭДО();
	КонтекстЭДОСервер.НачатьОпределениеТекстаЗаявления(АдресХранилища, Заявление);
	
КонецПроцедуры

Процедура НачатьОтправкуОблачногоЗаявленияВБазуЗаявок(АдресХранилища, Заявление) Экспорт
	
	КонтекстЭДОСервер = ДокументооборотСКОВызовСервера.ПолучитьОбработкуЭДО();
	КонтекстЭДОСервер.НачатьОтправкуОблачногоЗаявленияВБазуЗаявок(АдресХранилища, Заявление);
...
Функция ДоступнаАвторасшифровкаТранспортныхСообщений(УчетныеЗаписи) Экспорт
	...
	ИмяМодуляВременныйПрограммныйИнтерфейсБРО = "ВременныйПрограммныйИнтерфейсБРО";
	
	Если Метаданные.ОбщиеМодули.Найти(ИмяМодуляВременныйПрограммныйИнтерфейсБРО) <> Неопределено Тогда 
		МодульВременныйПрограммныйИнтерфейсБРО = ОбщегоНазначения.ОбщийМодуль(ИмяМодуляВременныйПрограммныйИнтерфейсБРО);
		МодульВременныйПрограммныйИнтерфейсБРО.ДоступнаАвторасшифровкаТранспортныхСообщений(УчетныеЗаписи, Результат);
	КонецЕсли;
