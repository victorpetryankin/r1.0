...
Функция ЗапросВТИнтервалыПоИнтервальномуРегистру(Знач ИмяРегистра, Знач МассивИзмерений, Знач ТекстОписанияПолей)
	...
	|	КОНЕЦПЕРИОДА(РегистрСведений.ДатаОкончания, ГОД) КАК КонецГода
	...
	|	КОНЕЦПЕРИОДА(ВТНовыйНаборЗаписей.ДействуетДо, ГОД)
	|ИЗ
	...
	|	МАКСИМУМ(ГодыИнтервалов.КонецГода) КАК КонецГода
