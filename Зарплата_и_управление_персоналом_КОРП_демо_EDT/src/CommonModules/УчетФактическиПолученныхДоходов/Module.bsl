...
// Созает временные таблицы движений регистрации доходов 
//
// Параметры:
//		Регистратор - ДокументСсылка - документ выплаты
//		Движения - коллекция движений регистратора
...
//		ДатаОперации - дата - дата, которой будет зарегистрировано движение.
//
Процедура ПоместитьВоВременныеТаблицыДвиженияРегистрацииНовойДатыДохода(Регистратор, Движения, МенеджерВременныхТаблиц, ДатаВыплаты, ДатаОперации) Экспорт
	
	ИменаРегистров = Новый Массив;
	ИменаРегистров.Добавить("СведенияОДоходахНДФЛ");
	ИменаРегистров.Добавить("РасчетыНалогоплательщиковСБюджетомПоНДФЛ");
	...
	НаборДвижений = Новый Структура;
	Для Каждого ИмяРегистра Из ИменаРегистров Цикл
		Если Не ПроведениеСервер.ЕстьНаборЗаписей(Движения, ИмяРегистра) Тогда
			Продолжить;
		КонецЕсли;
		НаборЗаписей = РегистрыНакопления[ИмяРегистра].СоздатьНаборЗаписей();
		НаборЗаписей.Отбор.Регистратор.Установить(ВременныйРегистратор);
		НаборДвижений.Вставить(ИмяРегистра, НаборЗаписей);
	КонецЦикла;
	
	Отказ = Ложь;
	УчетФактическиПолученныхДоходов.ЗарегистрироватьНовуюДатуПолученияДохода(Регистратор, НаборДвижений, МенеджерВременныхТаблиц, ДатаВыплаты, ДатаОперации, Отказ);
	Если Отказ Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого ИмяРегистра Из ИменаРегистров Цикл
		Если ПроведениеСервер.ЕстьДвижения(НаборДвижений, ИмяРегистра) Тогда
			ЗарплатаКадры.СоздатьВТПоНаборуЗаписей(МенеджерВременныхТаблиц, НаборДвижений[ИмяРегистра], Истина);
		КонецЕсли;
	КонецЦикла;
