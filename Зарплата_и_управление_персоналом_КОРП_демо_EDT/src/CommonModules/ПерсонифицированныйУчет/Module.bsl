...
Процедура УточнитьФорматСтраховогоНомераВСправочникеФизическиеЛица(ПараметрыОбновления = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ФизическиеЛицаОбъект = Выборка.Ссылка.ПолучитьОбъект();
...
Процедура УточнитьФорматРегистрационногоНомераВСправочникеОрганизации(ПараметрыОбновления = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Не ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ПодготовитьОбновлениеДанных(ПараметрыОбновления, "Справочник.Организации", "Ссылка", Выборка.Ссылка) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура УточнитьФорматРегистрационногоНомераВРегистреИсторияРегистрацийВОрганеПФР(ПараметрыОбновления = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Не ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ПодготовитьОбновлениеДанных(ПараметрыОбновления, "РегистрСведений.ИсторияРегистрацийВОрганеПФР", "Организация", Выборка.Организация) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ЗаполнитьАдрес(НаборЗаписейАдрес, СтрокаАдрес, СписокСтран, ФорматНеструктурированныйАдрес, ФорматИностранныйАдрес, ЗаменятьБуквуЕ = Ложь) Экспорт
	...
	Иначе
		...
		НаборЗаписейРоссийскийАдрес.Регион.Значение.ГеографическоеНазвание = РегламентированнаяОтчетность.ПолучитьИмяИАдресноеСокращение(ВРЕГ(СокрЛП(АдресСписком.Регион)), НаборЗаписейРоссийскийАдрес.Регион.Значение.Сокращение);
		Если ЗаменятьБуквуЕ Тогда
			ЗаменитьБуквуЁ(НаборЗаписейРоссийскийАдрес.Регион.Значение.ГеографическоеНазвание, "Регион");
		КонецЕсли;	
		
		НаборЗаписейРоссийскийАдрес.Район.Значение.ГеографическоеНазвание = РегламентированнаяОтчетность.ПолучитьИмяИАдресноеСокращение(ВРЕГ(СокрЛП(АдресСписком.Район)), НаборЗаписейРоссийскийАдрес.Район.Значение.Сокращение);
		Если ЗаменятьБуквуЕ Тогда
			ЗаменитьБуквуЁ(НаборЗаписейРоссийскийАдрес.Район.Значение.ГеографическоеНазвание, "Район");
		КонецЕсли;
		
		НаборЗаписейРоссийскийАдрес.Город.Значение.ГеографическоеНазвание = РегламентированнаяОтчетность.ПолучитьИмяИАдресноеСокращение(ВРЕГ(СокрЛП(АдресСписком.Город)), НаборЗаписейРоссийскийАдрес.Город.Значение.Сокращение);		
		Если ЗаменятьБуквуЕ Тогда	
			ЗаменитьБуквуЁ(НаборЗаписейРоссийскийАдрес.Город.Значение.ГеографическоеНазвание, "Город");
		КонецЕсли;
		
		НаборЗаписейРоссийскийАдрес.НаселенныйПункт.Значение.ГеографическоеНазвание = РегламентированнаяОтчетность.ПолучитьИмяИАдресноеСокращение(ВРЕГ(СокрЛП(АдресСписком.НаселенныйПункт)), НаборЗаписейРоссийскийАдрес.НаселенныйПункт.Значение.Сокращение);
		Если ЗаменятьБуквуЕ Тогда
			ЗаменитьБуквуЁ(НаборЗаписейРоссийскийАдрес.Город.Значение.ГеографическоеНазвание, "Населенный пункт");
		КонецЕсли;	
		НаборЗаписейРоссийскийАдрес.Улица.Значение.ГеографическоеНазвание = РегламентированнаяОтчетность.ПолучитьИмяИАдресноеСокращение(ВРЕГ(СокрЛП(АдресСписком.Улица)), НаборЗаписейРоссийскийАдрес.Улица.Значение.Сокращение);
		Если ЗаменятьБуквуЕ Тогда
...
Функция ПолучитьПредставлениеАдресаДляПФР(СтруктураАдрес) Экспорт 
	...
	Если СтруктураАдрес <> Неопределено Тогда 
		...
		Если Не ЗаПределамиРФ Тогда
			СокращениеРегион = "";
			Регион = РегламентированнаяОтчетность.ПолучитьИмяИАдресноеСокращение(ВРЕГ(СокрЛП(СтруктураАдрес.Регион)), СокращениеРегион);
			СокращениеРайон = "";
			Район = РегламентированнаяОтчетность.ПолучитьИмяИАдресноеСокращение(ВРЕГ(СокрЛП(СтруктураАдрес.Район)), СокращениеРайон);
			СокращениеГород = "";
			Город = РегламентированнаяОтчетность.ПолучитьИмяИАдресноеСокращение(ВРЕГ(СокрЛП(СтруктураАдрес.Город)), СокращениеГород);
			СокращениеНаселенныйПункт = "";
			НаселенныйПункт = РегламентированнаяОтчетность.ПолучитьИмяИАдресноеСокращение(ВРЕГ(СокрЛП(СтруктураАдрес.НаселенныйПункт)), СокращениеНаселенныйПункт);
			СокращениеУлица = "";
			Улица = РегламентированнаяОтчетность.ПолучитьИмяИАдресноеСокращение(ВРЕГ(СокрЛП(СтруктураАдрес.Улица)), СокращениеУлица);
