...
Процедура ВставитьПоказатели_ЕдиныйНалогНаВмененныйДоход(ТаблицаПоказателей, ОписаниеОтчета)
	...
	Если ОписаниеОтчета.ВыбраннаяФорма = "ФормаОтчета2018Кв1"
	 ИЛИ ОписаниеОтчета.ВыбраннаяФорма = "ФормаОтчета2017Кв1"
	 ИЛИ ОписаниеОтчета.ВыбраннаяФорма = "ФормаОтчета2016Кв1"
	 ИЛИ ОписаниеОтчета.ВыбраннаяФорма = "ФормаОтчета2015Кв1" Тогда
...
Процедура ИзменитьВерсиюФормата(Форма, ДопАтрибуты, ПараметрыВыгрузки) Экспорт
	Попытка
		Если СтрНачинаетсяС(Форма.ИмяФормы, "Внешний") Тогда
			Возврат;
		КонецЕсли;
		Если ТипЗнч(ДопАтрибуты) <> Тип("Структура") Или (Не ДопАтрибуты.Свойство("OKUD")) Или (Не ДопАтрибуты.Свойство("code")) Тогда 
			Возврат;
		КонецЕсли;
		
		Таблица = ПолучитьТекущуюТаблицуФорматов();
		Если ТипЗнч(Таблица) <> Тип("ТаблицаЗначений") Или Таблица.Количество() = 0 Тогда 
			Возврат;
		КонецЕсли;
		
		ОТЧ = Новый ОписаниеТипов("Число");
		ОтборСтроки = Таблица.НайтиСтроки(Новый Структура("ОКУД, Код, ТипПериодичности", ДопАтрибуты.OKUD, ДопАтрибуты.code, ОТЧ.ПривестиЗначение(ДопАтрибуты.idp)));
		Если ОтборСтроки.Количество() = 0 Тогда 
			ОтборСтроки = Таблица.НайтиСтроки(Новый Структура("ОКУД, Код, ТипПериодичности", ДопАтрибуты.OKUD, ДопАтрибуты.code, ДопАтрибуты.idp));
		КонецЕсли;
		Если ОтборСтроки.Количество() <> 1 Тогда 
			Возврат;
		КонецЕсли;
		
		ПараметрыВыгрузки.ВерсияШаблона = ОтборСтроки[0].Версия;
	Исключение
	КонецПопытки;
КонецПроцедуры
