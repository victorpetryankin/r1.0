			...
			Если Не ВыводитьПоРабочимМестам Тогда
				...
				Для Каждого СтрокаГоловногоСотрудника Из СтрокаМесяца.Строки Цикл
					...
					Если СтрокаГоловногоСотрудника.Строки.Количество() = 0 Тогда
						Продолжить;
					ИначеЕсли ГруппироватьПоИсточникамФинансирования Тогда
						
						ЕстьСтрокиДляВывода = Ложь;
						Для Каждого СтрокаИсточников Из СтрокаГоловногоСотрудника.Строки Цикл
							
							Если СтрокаИсточников.Строки.Количество() > 0 Тогда
								
								ЕстьСтрокиДляВывода = Истина;
								Прервать;
						...
						Если Не ЕстьСтрокиДляВывода Тогда
							Продолжить;
						КонецЕсли;
...
Процедура ВывестиСотрудникаРасчетныйЛисток(СтрокиГоловныхСотрудников, ДокументРезультатПромежуточный, ДокументРезультатСтраница, Макет, СоответствиеПользовательскихПолей, ДанныеПоНДФЛИВзносам, ГруппироватьПоИсточникамФинансирования, ВыводимыеГруппировки, НастройкиВывода, НаАванс, СвойстваРегистраторов)
	...
	Если ГруппироватьПоИсточникамФинансирования Тогда
		...
		Если СтрокаПервогоГоловногоСотрудника.Строки[0].Строки.Количество() = 0 Тогда
			Возврат;
		КонецЕсли;
	...
	Если ГруппироватьПоИсточникамФинансирования Тогда
		ИтогиПоГруппам = ВывестиГруппировкиПоСтатьямФинансирования(ДокументРезультат, Макет, НастройкиВывода, СтрокиГоловныхСотрудников, СоответствиеПользовательскихПолей, СвойстваРегистраторов);
	Иначе
		ИтогиПоГруппам = ВывестиТелоРасчетногоЛистка(ДокументРезультат, Макет, НастройкиВывода, СтрокиГоловныхСотрудников, СоответствиеПользовательскихПолей, СвойстваРегистраторов);
	КонецЕсли;
...
Функция ВывестиГруппировкиПоСтатьямФинансирования(ДокументРезультат, Макет, НастройкиВывода, СтрокиГоловныхСотрудников, СоответствиеПользовательскихПолей, СвойстваРегистраторов)
	...
	Для каждого СтатьяФинансирования Из СтатьиФинансирования Цикл
		...
		ИтогиПоСтатье = ВывестиТелоРасчетногоЛистка(ДокументРезультат, Макет, НастройкиВывода, СтрокиПоСтатьям, СоответствиеПользовательскихПолей, СвойстваРегистраторов);
...
Функция ВывестиТелоРасчетногоЛистка(ДокументРезультат, Макет, НастройкиВывода, СтрокиГоловныхСотрудников, СоответствиеПользовательскихПолей, СвойстваРегистраторов)
	...
	Для каждого СтрокаГоловногоСотрудника Из СтрокиГоловныхСотрудников Цикл
		...
		Для каждого СтрокаСотрудника Из СтрокаГоловногоСотрудника.Строки Цикл
			...
			Если СтрокаГруппы <> Неопределено И ЗначениеЗаполнено(СтрокаГруппы.Сумма) Тогда
...
Функция НастройкиВыводаРасчетногоЛистка(НастройкиОтчета, ПредварительныйПросмотр, НаАванс)
